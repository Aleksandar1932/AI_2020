...
if __name__ == '__main__':
    ...
    dataset = [[row[i] for i in range(0, len(row)) if i != COL_TO_RM] for row in dataset]
    ...
    # Init RF classifier and train it
    clf = RandomForestClassifier(criterion=CRITERIA, n_estimators=NUM_TREES)
    ...
    # Prediction on sample obtained from input
    input_sample = input().split(' ')
    del input_sample[COL_TO_RM]

    # Print accuracy and prediction
    print("Accuracy: {}".format(accuracy))
    print(clf.predict([input_sample])[0])
    print(clf.predict_proba([input_sample])[0])
