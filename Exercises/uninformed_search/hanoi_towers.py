...
def move_top_block(pilar_from, pilar_to, pilars):
    if len(pilars[pilar_from]) != 0 and (len(pilars[pilar_to]) == 0 or pilars[pilar_from][-1] <= pilars[pilar_to][-1]):
        pilars_list = list(list(pilar) for pilar in pilars)
        pilars_list[pilar_to].append(pilars_list[pilar_from].pop())
        return tuple(tuple(pilar) for pilar in pilars_list)
    return pilars


def parse_state(state_str):
    pilars = state_str.split(';')
    pilars_list = []
    for pilar in pilars:
        if len(pilar) == 0:
            pilars_list.append(tuple())
        else:
            pilars_list.append(tuple(int(block) for block in pilar.split(',')))

    return tuple(pilars_list)
...
class Hanoi(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(0, len(state)):
            for j in range(0, i):
                new_state = move_top_block(i, j, state)
                if new_state != state:
                    successors["MOVE TOP BLOCK FROM PILLAR {} TO PILLAR {}".format(i + 1, j + 1)] = new_state
...
if __name__ == '__main__':
    ...
    goal_state = parse_state(input())

    hanoi_problem = Hanoi(initial_state, goal_state)
    result = breadth_first_graph_search(hanoi_problem)
    result_steps = result.solution()

    # printing solution
    print("Number of steps required: {}".format(len(result_steps)))
    print("Solution:")
