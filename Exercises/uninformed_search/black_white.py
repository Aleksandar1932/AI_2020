"""
    ...
    2. Akcii vo sostojba:
        n^2 mozni akcii vo cekoja sostojba, t.e. pikanje na sekoja tocka

        akcija piknuvanje na pole def pick_point(x, y, state)
            (x, y) go pravi inverzna boja
            probuva gore dole levo desno;

        se iterira niz site polinja i se primenuva akcijata
"""
...
def pick_point(x, y, fields_tuple):
    fields_list = list(list(row) for row in fields_tuple)
    """
        y definira red
        x definira kolona
        fields_list[y][x]
    """
    # tockata
    fields_list[y][x] = 1 if fields_list[y][x] == 0 else 0


    # goren sosed
    if y - 1 >= 0:
        fields_list[y - 1][x] = 1 if fields_list[y - 1][x] == 0 else 0

    # dolen sosed
    if y + 1 < len(fields_list):
        fields_list[y + 1][x] = 1 if fields_list[y + 1][x] == 0 else 0

    # lev sosed
    if x - 1 >= 0:
        fields_list[y][x - 1] = 1 if fields_list[y][x - 1] == 0 else 0

    # desen sosed
    if x + 1 < len(fields_list):
        fields_list[y][x + 1] = 1 if fields_list[y][x + 1] == 0 else 0

    return tuple(tuple(row) for row in fields_list)
    ...
    def successor(self, state):
        ...
        for x in range(0, len(state)):
            for y in range(0, len(state)):
                new_state = pick_point(x, y, state)
                if new_state != state:
                    successors["x: {}, y: {}".format(x, y)] = new_state
...
if __name__ == '__main__':
    ...
    bw_problem = BlackWhite(generate_state(fields, n), generate_goal(n))

    print(breadth_first_graph_search(bw_problem).solution())
