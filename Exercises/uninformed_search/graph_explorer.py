...
"""
    1. Definiranje na akcii:
        Od edna pozicija vo druga pozicija moze da se pridvizis akko postoi link megju niv
        mozni pridvizuvanja se:
            Gore (tekovna - 4)
            Dolu (tekovna + 4)
            Levo (tekovna - 1)
            Desno (tekovna + 1)
    ...
    2. Definiranje na prostor na sostojbi:
        (player_position: int, (star1:int, star2:int),links:tuple)
        sekoj link e (a,b) tuple od tocki megju koi postoi istiot

        vkupno linkovi na pocetok: 21

        Mozni se 12 * 4 pocetni sostojbi:
            12 mesta za dzvezdi
            4 mesta za igrac

        Celna sostojba e tuple-ot od dzvezdi da bide prazen, t.e. site dzvezdi da se izedeni

"""


def link_exists(p_from, p_to, links_list):
    """
    funkcija koja proveruva dali postoi link megju dve tocki
    :param p_from:
    :param p_to:
    :param links_list:
    :return: True/ False
    """
    return (p_from, p_to) in links_list or (p_to, p_from) in links_list


def remove_link(p_from, p_to, links_tuple):
    """
    funkcija koja brisi link megju dve tocki, t.e. vrka nova lista so linkovi bez toj link
    :param p_from:
    :param p_to:
    :param links_tuple:
    :return: links_list with removed link between the two points
    """
    links_list = [list(el) for el in links_tuple if el != (p_from, p_to) and el != (p_to, p_from)]
    return tuple(tuple(el) for el in links_list)


def collect_star(player_pos, stars):
...
if __name__ == '__main__':
    ...
    initial_state = (player_position, (star_one_position, star_two_position), links)

    graph_explorer_problem = GraphExplorer(initial_state)

    result = breadth_first_graph_search(graph_explorer_problem)
